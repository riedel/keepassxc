...
namespace Phantom
{
    namespace
    {
        ...
        constexpr qreal LineEdit_Rounding = 0.0;
        constexpr qreal FrameFocusRect_Rounding = 1.0;
        constexpr qreal PushButton_Rounding = 0.0;
        constexpr qreal ToolButton_Rounding = 0.0;
        constexpr qreal ProgressBar_Rounding = 0.0;
        constexpr qreal GroupBox_Rounding = 0.0;
        constexpr qreal SliderGroove_Rounding = 0.0;
        constexpr qreal SliderHandle_Rounding = 0.0;
        ...
        namespace DeriveColors
        {
            ...
            {
                return adjustLightness(underlying, 0.04);
            }
...
void BaseStyle::polish(QApplication* app)
{
    ...
    for (const auto& path : getAppStyleSheetPaths()) {
        QFile extStylesheetFile(path);
        if (extStylesheetFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
            stylesheet.append(extStylesheetFile.readAll());
            extStylesheetFile.close();
        } else {
